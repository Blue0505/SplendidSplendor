...
def main(_):
  ...
  print("INFO STATE SHAPE", type(info_state_shape))
  agent = PPO(
      input_shape=tuple(info_state_shape),
  ...
  # self.post_step(reward, done)
      reward = torch.tensor(reward).to(device).view(-1)  # Flatten into a 1D array
      done = torch.tensor(done).to(device).view(-1)  # Flatten into a 1D array
      print(reward, done)
